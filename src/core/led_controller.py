...
class LEDController:
    ...
    GAMMA = 1.5  # LED dimming 인지 선형화 (감마 약하게 → 어두운 영역 PWM 더 큼)
    MIN_DUTY = 5.0  # LED 드라이버 dead zone 회피 (절충값)
    ...
    def _to_pwm_duty(self, brightness_percent: float) -> float:
        ...
        if b <= 0.0:
            return 0.0  # brightness=0은 완전 OFF
        pwm = (b ** self.GAMMA) * 100.0
        # 너무 낮은 duty는 LED가 깜빡임 → 최소 duty 보장
        if pwm < self.MIN_DUTY:
            pwm = self.MIN_DUTY
        return pwm
    ...
    def _set_pwm(self, pin: int, duty_percent: float):
        ...
        log.debug(f"PWM pin={pin} duty={duty:.1f}%")
        if self.h is None:
            return
        try:
